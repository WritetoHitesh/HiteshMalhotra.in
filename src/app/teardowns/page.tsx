...
import { getTeardowns } from "@/lib/mdx";
import { StaggerContainer, StaggerItem } from "@/components/MotionWrapper";

export const metadata = {
  title: "Teardowns | Hitesh Malhotra",
  description: "Product Teardowns and Architectural Audits.",
};
  ...
  return (
    <StaggerContainer className="max-w-5xl mx-auto px-6 py-24 min-h-screen">
      <StaggerItem className="mb-20 max-w-3xl">
        <h1 className="text-fluid-h2 font-bold tracking-tighter mb-6 uppercase">Product Teardowns</h1>
        <p className="text-xl md:text-2xl text-muted-foreground font-light tracking-wide leading-relaxed">
          Deep dives into the architecture, UX, and strategic decisions of market-leading products.
        </p>
      </StaggerItem>

      <StaggerItem className="flex flex-col border-t border-white/10">
        {teardowns.map((teardown, index) => (
          <Link key={teardown.slug} href={`/teardowns/${teardown.slug}`} className="group block w-full border-b border-white/5 py-12 transition-colors hover:bg-white/[0.02]">
